...
void ssd1306_WriteCommand(uint8_t byte) {
	HAL_StatusTypeDef status = HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &byte, 1, 100);
	if (status)
	{
	printf("OLED RX: status=%d error=0x%08lX state=0x%02X\r\n",
           status,
           HAL_I2C_GetError(&SSD1306_I2C_PORT),
           HAL_I2C_GetState(&SSD1306_I2C_PORT));
	}
}
...
{
	HAL_StatusTypeDef status = HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, buffer, buff_size, 100);
    if (status)
    {
	printf("OLED TX: status=%d error=0x%08lX state=0x%02X\r\n",
           status,
           HAL_I2C_GetError(&SSD1306_I2C_PORT),
           HAL_I2C_GetState(&SSD1306_I2C_PORT));
    }
...
void ssd1306_WriteCommand(uint8_t byte) {
    ...
    HAL_SPI_Transmit(&SSD1306_SPI_PORT, (uint8_t *) &byte, I2C_MEMADD_SIZE_8BIT, 100);
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    ...
    HAL_SPI_Transmit(&SSD1306_SPI_PORT, buffer, buff_size, 100);
    HAL_GPIO_WritePin(SSD1306_CS_Port, SSD1306_CS_Pin, GPIO_PIN_SET); // un-select OLED
}
